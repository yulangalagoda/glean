...
5. **New, raised by real validation:** the judge itself made real errors in the first real run (see Validation) — is a documented per-target caveat in `glean eval`'s own output enough, or does this need a second judge-reliability metric (e.g. spot-checking a sample of judge verdicts by hand) before the stage-2 number can be trusted at face value? Not resolved here.
